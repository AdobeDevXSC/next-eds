...
export function normalizeHtml(html, origin) {
  ...
  // Strip interactive on* attributes (onclick, onerror, onload, ...). This output is served
  // as text/html to anonymous browsers when preview=true, so inline event handlers must not
  // survive. (javascript: URLs are separately defanged below by absolutize, which rewrites
  // them to `${origin}/javascript:...` — an inert path, not an executable scheme.)
