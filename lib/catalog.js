 ...
 * @typedef {{ id: string, name: string, priceCents: number, default: boolean,
 *   color: string }} Ingredient
 */
...
/** Derive a stable slug id from an ingredient's authored name — e.g. "Salami & Capicola" ->
 * "salami-capicola", "Maple-pepper Glaze" -> "maple-pepper-glaze". Not authored: the Ingredients
 * block has no id column, so the app derives one to key builder selection/React lists. Matches
 * the ids the previous hand-authored content/builder-palette.json used. */
function deriveIngredientId(name) {
...
export async function getMenu() {
  ...
  return [...feed.entries()]
    .map(([path, row]) => {
      ...
      const image = decodeEntities(row.image || '').trim();
      ...
      const rawCategory = decodeEntities(row.category || '').trim();
      const category = CATEGORIES.includes(rawCategory) ? rawCategory : 'signature';
      const tags = (row.tags || '').split(',')
        .map((t) => decodeEntities(t).trim())
        .filter(Boolean);
      const special = decodeEntities(row.special || '').trim();
...
export async function getIngredients() {
  ...
  block.rows.forEach((row) => {
    ...
    const id = deriveIngredientId(name);
    if (!id) return;
    palette[type].push({
      id,
      name,
      ...
      // Color (col 4) is presentational only, unlike the required fields above — a row
      // missing it is still kept (renderable with an empty swatch) rather than dropped.
...
// Category display metadata for the /build palette: label/note/select-mode and rendered order.
// This is UI structure, not authored content, so it lives in code rather than the Ingredients
// block — composed with authored items in getBuilderPalette() below. A category absent here
// (e.g. 'extra') is simply never included in the composed palette, authored or not.
const BUILDER_CATEGORIES = [
  { key: 'bread', label: 'Bread', note: 'pick one', select: 'one' },
  { key: 'protein', label: 'Protein', note: 'stack as many as you like', select: 'many' },
  { key: 'cheese', label: 'Cheese', note: 'stack as many as you like', select: 'many' },
  { key: 'veg', label: 'Veg & pickles', note: 'free-ish', select: 'many' },
  { key: 'sauce', label: 'Sauce', note: 'one or two', select: 'many' },
];

/**
 * Compose the /build page's palette: authored ingredients (getIngredients()) grouped under the
 * fixed category display config above, in that order. A category with zero authored items is
 * dropped from the output entirely. Never throws — getIngredients() already degrades to an
 * empty palette on error/404, which composes here to an empty array.
 * @returns {Promise<Array<{ key: string, label: string, note: string, select: string,
 *   items: Ingredient[] }>>}
 */
export async function getBuilderPalette() {
  const ingredients = await getIngredients();
  return BUILDER_CATEGORIES
    .map((category) => ({ ...category, items: ingredients[category.key] || [] }))
