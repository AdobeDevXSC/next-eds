 ...
 * size. Cached with a plain time-based revalidate (ISR) — no cache tag — so pages stay
 * statically generated and the feed refreshes on its own schedule. Returns an empty Map if
 * the feed is missing or malformed, so callers degrade gracefully.
