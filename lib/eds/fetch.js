// Content origin for EDS-delivered markup (*.aem.page = preview, *.aem.live = published).
//   - Production: wrangler.jsonc `vars.EDS_ORIGIN` is bound onto process.env by the deployed
//     Worker, so this resolves to the .live origin (required for EDS push invalidation).
//   - Local `next dev`: OpenNext does NOT inject wrangler vars / .env.local / .dev.vars into the
//     Node process the RSC runs in, so process.env.EDS_ORIGIN is undefined here and this falls back
//     to the .page default. That's intentional — local dev reads *preview* content.
