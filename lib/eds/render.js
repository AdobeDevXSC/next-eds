...
import { decorateButtonsHtml } from './buttons.js';
...
export function renderNode(node, key) {
  ...
  // default content (headings, paragraphs, lists, …). Run the server-side button decoration
  // so a standalone bold/italic CTA link renders as a button (the client decorateButtons the
  // RSC path replaces never runs here).
  return (
    // eslint-disable-next-line react/no-danger
    <div key={key} dangerouslySetInnerHTML={{ __html: decorateButtonsHtml(node.html) }} />
  );
}
