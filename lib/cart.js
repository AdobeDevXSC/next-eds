...
// into the user's cart and clears the guest cookie. User and guest carts live in separate KV
// key namespaces (`cart:user:<id>` / `cart:guest:<id>`) so a forged guest-cart cookie value
// can never collide with — and read or delete — a real user's cart. See
...
export async function getCartKey(user) {
  return user ? `cart:user:${user.id}` : `cart:guest:${await getOrCreateGuestCartId()}`;
}
...
export async function mergeGuestCartIntoUser(userId) {
  ...
  const kv = getKv();
  const guestRaw = await kv.get(`cart:guest:${guestId}`);
  if (guestRaw) {
    const guestItems = JSON.parse(guestRaw);
    const userRaw = await kv.get(`cart:user:${userId}`);
    const userItems = userRaw ? JSON.parse(userRaw) : [];
    await kv.put(`cart:user:${userId}`, JSON.stringify([...userItems, ...guestItems]));
    await kv.delete(`cart:guest:${guestId}`);
  }
